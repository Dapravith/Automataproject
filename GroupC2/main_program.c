...
main(){
    ...
    if (GetFontSize(h, &size)){
        /* Grow by 50% */
        size.X += (SHORT)(size.X * .4);
        size.Y += (SHORT)(size.Y * .4);
        SetFontSize(h, size);
    }
    ...
    //deleteDatabase(); //function for delete database
    char option;
    while(1){
        cls();
        char mainmenu[4][50]={"1.   Load FA From Database ","2.   Create new FA ","3.   Delete FA from Database ","4.   Exit program "};
        ...
        if(num==1) {
               ...
               //loading();
                int fa_id;
                char *sql;
                char sql1[200],sql2[200],sql3[200],sql4[200];
                header("***FA FILE***");
                sql = "SELECT * FROM FA;";
                sqlite3_exec(db, sql, loadFa, 0, &zErrMsg);

                printf("\n\t\t\t\t\t\tEnter File ID: ");SetColor(1);
                scanf("%d",&fa_id);SetColor(8);
                ...
                while(1){
                    ...
                    char submenu[5][50]={"1.   Display Transition Table","2.   Test FA String","3.   Convert NFA to DFA","4.   Minimize DFA","5.   Back to Main Menu" };
                    char head1[50]="====>SUB-MENU<====";
                    printf("\n");
                    int m2=sizeof(submenu)/sizeof(submenu[0]);
                    int num1=choice(submenu,head1,m2);
                    ...
                    switch(num1) {
                        ...
                        case 2:
                            ...
                            while(1){
                                ...
                                printf("\n\n");
                                printf("\n\n\t\t\t\t\t[input 0 to go back]");
                                printf("\n\t\t\t\t\tinput a string:");SetColor(3);
                                scanf("%s",&input);SetColor(8);
